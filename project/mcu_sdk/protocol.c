 ...
 * @return Null
 */
extern rt_device_t serial_tuya; 
void uart_transmit_output(unsigned char value)
{
//    #error "请将MCU串口发送函数填入该函数,并删除该行"
    rt_device_write(serial_tuya, 0, &value, 1);
/*
    //Example:
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,95); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_BATTERY_STATE,2); //枚举型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,88); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_CHARGE_STATE,1); //BOOL型数据上报;
    mcu_dp_value_update(DPID_MAXTEMP_SET,40); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,10); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,99); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,50); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_TEMP_ALARM,30); //枚举型数据上报;
    mcu_dp_enum_update(DPID_HUM_ALARM,96); //枚举型数据上报;
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,22); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,33); //VALUE型数据上报;
    ...
    */
    if(time[0] == 1) {
        ...
        gWriteTime(time);
			...
			rt_kprintf("can not get tuya time\n");
...
//    #error "请自行完成打开天气功能返回数据处理代码,完成后请删除该行"
    ...
    if(res == 1) {
			rt_kprintf("open weather succeed\n");
        //打开天气返回成功
    }else if(res == 0) {
			rt_kprintf("open weather failed\n");
        //打开天气返回失败
        //获取错误码
        err_num = err; 
    }
